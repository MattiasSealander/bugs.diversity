...
generate_bcDiversity_plots <- function(list_of_lists) {
  ...
  for (i in seq_along(list_of_lists)) {
    ...
    for (j in seq_along(sublist)) {
      ...
      # Shannon is a generic method. If its first argument is an abundance vector, an integer vector or a numeric vector which does not sum
      # to 1, the bias corrected function bcShannon is called.
      if (!is.null(mc)) {
        shannon <- tryCatch({
          Shannon(Ps = mc$Ps)
        }, error = function(e) {
        ...
        # Simpson is a generic method. If its first argument is an abundance vector, an integer vector or a numeric vector which does not sum
        # to 1, the bias corrected function bcSimpson is called.
        simpson <- tryCatch({
          Simpson(Ps = mc$Ps)
        }, error = function(e) {
    ...
    p <- ggplot() +
      ...
      labs(
        title = paste("Diversity indices for", top_list_names[i]),
        x = "Diversity Value",
        y = "Time (Years BP)",
        fill = "Period"
      ) +
    ...
    file_name <- paste0("003-", safe_name, "-diversity-regional.jpg")
...
mc <- MetaCommunity(Listdf2[[1]][[9]])

# bias-corrected Richness
bcRichness(Ns = mc$Ns, Correction = "Best")

#effective number of species equal to Hill (1973) numbers
Diversity(mc$Nsi, q = 2)
