...
bugs <- 
  ...
  distinct(region, sample, sample_group, site, age_older, age_younger) %>% 
    mutate(
    start = age_younger,
    end   = age_older
  )
...
# ==============================================================
overlap_hits <- tibble(
  sample  = bugs$sample[queryHits(overlaps)],
  sample_group = bugs$sample_group[queryHits(overlaps)],
  site    = bugs$site[queryHits(overlaps)],
  bin_start = age_bins$start[subjectHits(overlaps)],
  bin_end   = age_bins$end[subjectHits(overlaps)],
  region = bugs$region[queryHits(overlaps)]
)

# Filter out unique nr. of sites
sites_eu <- 
  overlap_hits %>%
  select(site) %>% 
  distinct()
...
# Filter out unique nr. of samples
samples_brit <-  
  overlap_hits %>%
  filter(region == "British/Irish Isles") %>% 
...
# Late Glacial nr. of samples
LG_samples <-  
  overlap_hits %>%
  filter(
    region == "British/Irish Isles",
    between(bin_end, 12000, 16000)) %>% 
  select(site, sample_group, sample) %>% 
  distinct()

# Early Holocene nr. of samples
EH_samples <-  
  overlap_hits %>%
  filter(
    region == "British/Irish Isles",
    between(bin_end, 5500, 11500)) %>%
  select(site, sample_group, sample) %>% 
  distinct()

# Late Holocene nr. of samples
LH_samples <-  
  overlap_hits %>%
  filter(
    region == "British/Irish Isles",
    between(bin_end, 500, 5500)) %>% 
  select(site, sample_group, sample) %>% 
  distinct()
```
