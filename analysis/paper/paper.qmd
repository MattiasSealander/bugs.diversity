...
institute:
  ...
  - LQR:
      name: Laboratory of Quaternary Research, Nature Research Centre
      address: Akademijos g. 2, 08412 Vilnius, Lithuania
  - SU:
...
# 1. Read data
# --------------------------------------------------------------
# Read plots and data produced in other scripts
# ==============================================================

# Generate d18O plot
quietly(source(here::here("analysis", "paper", "006-oxygen-climate.R"), encoding = "UTF-8"))
# Read d18O plot
p_d18 <- readRDS(here::here("analysis", "data", "derived_data", "d18O_plot.rds"))

# Generate pollen plot
quietly(source(here::here("analysis", "paper", "006-pollen-diversity.R"), encoding = "UTF-8"))
# Read pollen plot
p_pollen <- readRDS(here::here("analysis", "data", "derived_data", "neotoma_pollen.rds"))

# Read gamma diversity plots
p_shan <- readRDS(here::here("analysis", "data", "derived_data", "gamma_shan.rds"))
p_simp <- readRDS(here::here("analysis", "data", "derived_data", "gamma_simp.rds"))

# Read ecocode data
eco_results <- readRDS(here::here("analysis", "data", "derived_data", "ecocode_results.rds"))

# ==============================================================
# 2. Generate ecocode plot
# --------------------------------------------------------------
# Read habitat reconstruction data, and filter to abundance-weighted results
# For reconstruction based on unique taxa only, filter to "No Abundance"
# ==============================================================
...
eco_results$ecocode <- factor(eco_results$ecocode, levels = ecocode_levels)
...
# 3.1 Bottom legend for gamma diversity
gamma_legend <- p_shan +
  guides(
...
# 3.2 Assemble legend
# Extract gamma and ecocode legends
gamma_bottom <- cowplot::get_legend(gamma_legend)
eco_bottom <- cowplot::get_legend(p_eco)

# Trim extraneous whitespace from the legend gtables
gamma_bottom <- gtable::gtable_trim(gamma_bottom)
...
# 3.3 Add titles and guides
p_shan2 <- p_shan +
  labs(title = "Gamma Diversity",
         subtitle = "Shannon") +
    guides(
    shape    = guide_legend(nrow = 1, title = NULL, position = "top"),
...
# 3.4 Assemble figure
# Left beetle data plots
left_group <- (p_shan2 + p_simp2 + p_eco2) +
  plot_layout(ncol = 3)

# Right pollen and climate plots
