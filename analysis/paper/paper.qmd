...
LG_samples <- hits %>%
  filter(region == "British/Irish Isles",
         bin_end >= 10500 & bin_start <= 16000) %>% 
  distinct(sample_id)

# Early Holocene samples UK/Ireland
EH_samples <- hits %>%
  filter(region == "British/Irish Isles",
         bin_end >= 6500 & bin_start <= 10000) %>%
  distinct(sample_id)
  
# Late Holocene samples UK/Ireland
LH_samples <- hits %>%
  filter(region == "British/Irish Isles",
         bin_end >= 0 & bin_start <= 6000) %>% 
  distinct(sample_id)
```
...
quietly(source(here::here("analysis", "paper", "006-ecocodes-britain-coleoptera.R"), encoding = "UTF-8"))
...
combined <- p_div + p_eco + p_d18 +   plot_layout(ncol = 3, widths = c(1, .6, .5))
