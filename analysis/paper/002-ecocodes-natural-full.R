...
time.mat <- bugs %>%
  select(sample, sample_group, site, age_older, age_younger, context) %>%
...
fig <- ggplot(eco_combined, aes(x = end, y = value, fill = ecocode)) +
  ...
  scale_x_reverse(limits = c(16000, -500)) +
  coord_flip() +
  ...
  labs(y = "%SumRep", x = "Age (BP)")

# ---- 14. Save figure ----
ggsave(
  "004-ecocodes-natural-full.jpg",
  fig,
  device = "jpg",
...
message("✅ Ecological composition analysis completed and figure saved: 004-ecocodes-natural-full.jpg")
