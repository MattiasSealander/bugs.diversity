# ==============================================================
# Description:
#   This script computes richness and coverage metrics (Observed Richness,
...
# ==============================================================
# Function: generate_richness_raw
# ==============================================================
# Description:
#   Computes richness estimation and coverage metrics for each time slice
#
# Arguments:
...
# Returns:
#   List containing:
#     - $plot: ggplot object of richness and coverage metrics
#     - $results: tidy data.frame of metric values per time slice
# ==============================================================

generate_richness_raw <- function(Listdf, rects,
  ...
  for (time_name in names(Listdf)) {
    ...
    #   Leaving weights out MetaCommunity() will give samples equal weights
    Weights <- colSums(comm_num)
    Weights <- Weights / sum(Weights)
    MC <- tryCatch(entropart::MetaCommunity(as.matrix(comm_num), Weights), error = function(e) NULL)

    richness <- NA
    # #Richness is a generic method. If its
    # first argument is an abundance vector, an integer vector or a numeric vector which does not sum to 1,
    # the bias corrected function bcRichness is called" (from entropart documentation)
    if (!is.null(MC)) {
      richness <- tryCatch(entropart::Richness(MC$Ns, Correction = "Chao1"),
                           error = function(e) NA)
    }
    ...
    add_metric("Estimated Richness", richness)
    add_metric("Observed Richness", obs_rich)
    add_metric("Coverage", cov)
  }

  if (nrow(results) == 0) {
    message("No valid time slices found for richness estimation.")
    return(NULL)
  }
  ...
  results$Metric <- factor(results$Metric,
                           levels = c("Observed Richness", "Coverage", "Estimated Richness"))
  ...
  p <- ggplot() +
    ...
    labs(
      title = "Richness estimation (British/Irish Isles)",
      x = "Value",
      y = "Time (Years BP)",
      fill = "Time Periods"
    ) +
  ...
  # ---- Save plot ----
  ggsave(
    filename = "002-richness-estimation-britain.jpg",
    plot = p,
    path = output_dir,
    width = 3300, height = 4200, units = "px", dpi = 300
  )

  message("✅ Richness estimation plot generated successfully.")
  return(list(plot = p, results = results))
}


# ---- 14. Run analysis ----
res_rich <- generate_richness_raw(
  Listdf,
  rects,
  output_dir = here::here("analysis", "figures")
)
