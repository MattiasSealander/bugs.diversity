...
bbox <- st_bbox(
  c(xmin= -11.5, ymin= 49.5, xmax= 2.5, ymax= 62.5), crs = 4326) %>%
  st_as_sfc()

# ---- 3. Filter and categorize sites ----
# Filter for stratigraphic sequences (natural deposits), focus on Britain, exclude temporary samples (BugsPresence), age limits, and Greenland
sites <-
  bugs %>%
...
sites.sf <-
  st_as_sf(sites, coords = c("longitude", "latitude"), crs = 4326)
...
message("✅ Sites have been plotted and figure saved: '001-sites-britain.jpg'")
